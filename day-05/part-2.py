...
def fill_grid(grid, line):
    if line[1] == line[3] and line[0] != line[2]:  # vertical
        min_x = min(line[0], line[2])
        max_x = max(line[0], line[2])
        for i in range(min_x, max_x + 1):
            grid[i][line[3]] += 1
    elif line[0] == line[2] and line[1] != line[3]:  # horizontal
        min_y = min(line[1], line[3])
        max_y = max(line[1], line[3])
        for i in range(min_y, max_y + 1):
            grid[line[2]][i] += 1
    elif line[2] - line[0] == (line[3] - line[1]) * -1:  # diagonal /
        for i in range(0, abs(line[2] - line[0]) + 1):
            grid[i + min(line[2], line[0])][max(line[3], line[1]) - i] += 1
    elif line[2] - line[0] == line[3] - line[1]:  # diagonal \
        for i in range(0, abs(line[2] - line[0]) + 1):
            grid[i + min(line[2], line[0])][i + min(line[3], line[1])] += 1
